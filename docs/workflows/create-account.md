...
sequenceDiagram
    ...
    EmailVerification ->> Account: verificationCreated(email)
...
sequenceDiagram
    ...
    Note over EmailVerification: STATUS: ACTIVATION_LINK_NOT_SENT
    EmailVerification -->> Account: verificationCreated()
    alt if offline
        Note over EmailVerification: stop here
    end
    EmailVerification ->> EmailVerificationEffect: sendActivationLinkRequested()
    %% Note over EmailVerificationEffect: save retrieveToken in local storage
    EmailVerificationEffect -->> EmailVerification: sendActivationLinkSucceeded()
    Note over EmailVerification: STATUS: ACTIVATION_LINK_SENT_BUT_NOT_CLICKED
    EmailVerification ->> EmailVerificationEffect: scheduledRefreshTaskRequested()
    loop every 60 seconds until user has verified the email or the verification has expired
        ...
        EmailVerificationEffect ->> EmailVerification: scheduledRefreshTaskTimeUp()
...
sequenceDiagram
    ...
    EmailVerificationEffect ->> EmailVerificationService: createRequested(email)
    Note over EmailVerificationService: status:ACTIVATION_LINK_NOT_SENT
    EmailVerificationService ->> EmailVerificationStorage: save()
    EmailVerificationEffect ->> EmailVerificationService: saveActivationLinkRequested()
    EmailVerificationService ->> EmailVerificationStorage: load()
    Note over EmailVerificationService: status:ACTIVATION_LINK_NOT_SENT
    EmailVerificationService ->> EmailVerificationApi: sendActivationLink()
    Note over EmailVerificationService: status:ACTIVATION_LINK_SENT_BUT_NOT_CLICKED + retrieveToken
    EmailVerificationService ->> EmailVerificationStorage: save()
    EmailVerificationEffect ->> EmailVerificationService: refreshRequested()
    EmailVerificationService ->> EmailVerificationStorage: load()
    Note over EmailVerificationService: status:ACTIVATION_LINK_SENT_BUT_NOT_CLICKED + retrieveToken
...
sequenceDiagram
    ...
    Note over EmailVerificationService: status:ACTIVATION_LINK_SENT_BUT_NOT_CLICKED
