...
const createOwnChainedEvents = (component, componentPath) => {
    ...
    chainedEvents.forEach(({ onEvent, thenDispatch }) => {
        ...
        validateNotEmptyString({ "onEvent.eventType": onEvent.eventType }, { componentPath });
        validateNotNullObject({ thenDispatch }, { componentPath });
        validateNotEmptyString({
            "thenDispatch.eventType": thenDispatch.eventType,
        }, { componentPath });
    });
    return (previousEvent, globalState) => (component.chainedEvents ?? [])
        .filter(({ onEvent }) => isEqual(resolveComponentPath(componentPath, onEvent.childComponentPath), previousEvent.componentPath))
...
const createChildrenChainedEvents = (component, componentPath) => (previousEvent, globalState) => {
    return Object.entries(component.children ?? {}).flatMap(([childName, childComponent]) => eventChainFactory(childComponent, [...componentPath, childName])(previousEvent, globalState));
};
