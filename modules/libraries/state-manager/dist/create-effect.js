...
import { validateFunction, validateNotEmptyString, validateNotNullObject, } from "@softersoftware/functions/assert.functions";
export const createEffect = ({ afterEvent, onComponent, then }) => {
    validateNotNullObject({ afterEvent }, { afterEvent, onComponent, then });
    validateNotEmptyString({ triggeringEventType: afterEvent.eventType }, { afterEvent, onComponent, then });
    ...
    return (event) => isMatch(event, { triggeringEventType, componentPath }) &&
        effectFunction(event.payload);
};
const isMatch = (event, { triggeringEventType, componentPath }) => {
    return (event.eventType === triggeringEventType &&
