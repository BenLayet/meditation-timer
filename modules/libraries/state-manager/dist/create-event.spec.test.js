...
describe("createEvent", () => {
    ...
    it("should throw an error if the payload does not match the payload shape", () => {
        ...
        expect(() => createEvent({ payloadShape, eventType }, componentPath, payload)).toThrow("Expected property `key2` to be of type `number` but received type `string` in object `::while creating event InvalidComponent:InvalidEvent");
    });
    it("should throw an error if required keys are missing in the payload", () => {
        ...
        expect(() => createEvent({ payloadShape, eventType }, componentPath, payload)).toThrow('Expected property `key2` to be of type `number` but received type `undefined` in object `::while creating event MissingKeyComponent:MissingKeyEvent with payload {"key1":"Hello"}`');
    });
    ...
    it("should take isNewCycle=true into account", () => {
        const eventType = "TestEvent";
        const componentPath = [];
        const result = createEvent({ eventType, isNewCycle: true }, componentPath, {});
        expect(result).toEqual({
            componentPath: [],
            eventType: "TestEvent",
            payload: {},
