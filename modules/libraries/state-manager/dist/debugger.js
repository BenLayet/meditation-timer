const writeStateAtPath = (globalState, componentPath, localState) => componentPath
    .slice()
    .reverse()
    .reduce((childState, childName, index) => {
    const { children, ownState } = getStateAtPath(globalState, componentPath.slice(0, componentPath.length - 1 - index));
    return {
        children: {
            ...children,
            [childName]: {
...
const getStateAtPath = (globalState, componentPath = []) => componentPath.reduce((result, childName) => result.children[childName], globalState);
const getValueAtPath = (globalObject, path) => path.reduce((res, property) => res[property], globalObject);
const writeValueAtPath = (globalObject, path, patch) => path
    .slice()
    .reverse()
...
const parseKey = (key, state) => key.split(".").reduce((res, property) => {
