...
Given(/^I have started a meditation session$/, function () {
    this.vm().children.meditationSession.dispatchers.startRequested({currentTimeInSeconds: CURRENT_TIME_IN_SECONDS});
});
...
Then(/^the sleep mode should be reenabled$/, function () {
    expect(this.eventWasSent(meditationSessionEvents.disableSleepModeRequested)).to.be.true;
    expect(this.eventWasSent(meditationSessionEvents.enableSleepModeRequested)).to.be.true;
});
