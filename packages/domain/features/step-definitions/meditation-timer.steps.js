...
Then(
  /^the timer should display (\d{2}:\d{2})$/,
  function (expectedDisplayedTime) {
    const actual =
      this.vm().children.meditationSession.selectors.meditationRemainingTime();
    expect(actual).to.equal(expectedDisplayedTime);
  },
);
Then(
  /^the meditation timer (should|should not) (?:start|be) running$/,
  function (should) {
    const expected = should === "should";
    const actual =
      this.vm().children.meditationSession.selectors.meditationIsRunning();
    expect(actual).to.equal(expected);
  },
);
Then(/^a gong sound should be played$/, function () {
  expect(this.eventWasSent(meditationTimerAppEvents.gongPlayRequested)).to.be
    .true;
});
