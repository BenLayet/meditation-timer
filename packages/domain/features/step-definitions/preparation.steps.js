...
import {CURRENT_TIME_IN_SECONDS, PREPARATION_DURATION_IN_SECONDS} from "../support/test-constants.js";


Given(/^the preparation has started$/, function () {
    this.patchState("meditationSession.preparation.startedTimeInSeconds", CURRENT_TIME_IN_SECONDS);
});
Given(/^there are (\d+) seconds left in the preparation$/, function (remainingSeconds) {
    this.patchState("meditationSession.preparation.remainingSeconds", remainingSeconds);
});
...
When(/^the preparation duration has elapsed$/, function () {
    ...
    this.vm().children.meditationSession.children.preparation.dispatchers.timerTicked({currentTimeInSeconds});
});
...
Then(/^the preparation timer should (start|stop) running$/, function (start) {
    ...
    const actual = this.vm().children.meditationSession.children.preparation.selectors.isRunning();
    expect(actual).to.equal(expected);
});
Then(/^the preparation timer should display (\d\d:\d\d)$/, function (remainingTime) {
    const actual = this.vm().children.meditationSession.children.preparation.selectors.remainingTime();
