...
Given(
  ...
  function (days, minutes) {
    this.meditationStorage.meditations = Array.from(
      { length: days },
      (_, i) => ({
...
Then(/^the meditation session should be saved$/, function () {
  expect(this.eventWasSent(actualMeditationEvents.saveRequested)).to.be.true;
});
Then(/^my new daily streak should be displayed$/, function () {
  expect(
    ...
    "saveSucceeded should have been called",
  ).to.be.true;
  expect(
    this.eventWasSent({
    ...
    "statisticsRequested should have been called",
  ).to.be.true;
  ...
  actual = this.vm().children.statistics.selectors.hourCountThisWeek();
  expect(actual.count, "hourCountThisWeek").to.equal(3);

  actual = this.vm().children.statistics.selectors.dailyStreak();
  expect(actual, "daily streak").to.equal(5);

  actual =
    this.vm().children.statistics.selectors.shouldDailyStreakBeDisplayed();
  expect(actual, "daily streak should be displayed").to.be.true;
});
Then(/^the statistics are shown$/, function () {
  const actual = this.vm().selectors.currentPage();
