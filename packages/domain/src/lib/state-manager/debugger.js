const writeStateAtPath = (globalState, componentPath, localState) =>
    componentPath.slice().reverse().reduce((childState, childName, index) => {
        const {children, ownState} = getStateAtPath(globalState,
...
const getStateAtPath = (globalState, componentPath = []) => componentPath.reduce((result, childName) =>
    result.children[childName], globalState);
const getValueAtPath = (globalObject, path) => path.reduce((res, property) => res[property], globalObject);
const writeValueAtPath = (globalObject, path, patch) => path.slice().reverse()
    .reduce((res, property, index) => ({
        ...getValueAtPath(globalObject, path.slice(0, path.length - 1 - index)),
        [property]: res
    }), patch);
const parseKey = (key, state) => key.split('.')
    .reduce((res, property) => {
    ...
    }, {newState: state, componentPath: [], remainderPath: [], pathCompleted: false});
export const statePatcher = (stateManager) => (key, value) => {
    ...
    const newLocalState = writeValueAtPath(localState.ownState, remainderPath, value);
    const newState = writeStateAtPath(stateManager.state, componentPath, newLocalState)
    stateManager.dispatch({eventType: 'FORCE_STATE', payload: {newState}, componentPath: []});
