...
export const providers = {
  indexedDb: async ({ schema = meditationsIndexedDbSchema }) => createIndexedDb(schema),
  transactionService: ({ indexedDb }) => new TransactionService(indexedDb),
  keyValueStore: ({ storeName = keyValueStoreName }) => new KeyValueStore(storeName),
  keyValueStorageService: ({
    keyValueStore,
    transactionService
  }) => new KeyValueStorageService(keyValueStore, transactionService),
  meditationStore: ({ storeName = meditationStoreName }) => new CollectionStore(storeName),
  pendingEventStore: ({ storeName = pendingEventStoreName }) => new CollectionStore(storeName),
  ...
  pendingEventService: ({ transactionService, pendingEventStore, eventProcessor, synchronizationTaskService }) =>
    new PendingEventService(transactionService, pendingEventStore, eventProcessor, synchronizationTaskService),
  meditationService: ({ indexedDb, meditationStore }) => new MeditationService(indexedDb, meditationStore),
