...
export class EmailVerificationService {
  ...
  async createEmailVerification (email) {
    ...
    const emailVerification = await this.emailVerificationApi.createEmailVerification(email)
  ...
  async getEmailVerification (email) {
    ...
    switch (emailVerification.status) {
      case emailVerificationStatus.NOT_REQUESTED:
        emailVerification = await this.emailVerificationApi.createEmailVerification(email)
        break
      case emailVerificationStatus.REQUESTED:
        emailVerification = await this.retrieveEmailVerification(emailVerification)
        break
  ...
  async retrieveEmailVerification (emailVerification) {
    try {
      const remoteEmailVerification = await this.emailVerificationApi.retrieveEmailVerification(emailVerification.uuid, emailVerification.retrieveToken)
  ...
  async storeLocalEmailVerification (emailVerification) {
    ...
    await this.keyValueStorageService.set('emailVerification', emailVerification)
  }
